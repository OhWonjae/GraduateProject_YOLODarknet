...
extern "C" void draw_detections_cv_v3(mat_cv* mat, detection *dets, int num, float thresh, char **names, image **alphabet, int classes, int ext_output)
{
    ...
    int mask_count;
    int nonmask_count;
    char mask_count_buffer[5];
    char nonmask_count_buffer[5];
    FILE* fp = fopen("C:\\mask_count.txt", "r");
    fgets(mask_count_buffer, sizeof(mask_count_buffer), fp);
    mask_count = atoi(mask_count_buffer);
    fclose(fp);
    fp = fopen("C:\\nonmask_count.txt", "r");
    fgets(nonmask_count_buffer, sizeof(nonmask_count_buffer), fp);
    nonmask_count = atoi(nonmask_count_buffer);
    fclose(fp);
    // codeend
    try {
            ...
            for (j = 0; j < classes; ++j) {
                ...
                if (dets[i].prob[j] > thresh && show) {
                    if (class_id < 0) {
                        ...
                        //code start
                        if (mask_count < dectime && nonmask_count<dectime)
                        {
                            ...
                            if (!strcmp(names[j], "good"))
                            {
                                ...
                                {
                                    //인식중입니다 목소리 출력
                                    PlaySound("C:\\recostart.wav", NULL, SND_FILENAME | SND_ASYNC | SND_LOOP);
                                    int reclen = 100;
                                    while (reclen > 0)
                                    {
                                        reclen--;
                                ...
                                mask_count++;
                                sprintf(mask_count_buffer, "%d", mask_count);
                                FILE* fp = fopen("C:\\mask_count.txt", "w");
                                fwrite(mask_count_buffer, strlen(mask_count_buffer), 1, fp);
                                fclose(fp);

                                sprintf(nonmask_count_buffer, "%d", 0);
                                fp = fopen("C:\\nonmask_count.txt", "w");
                                fwrite(nonmask_count_buffer, strlen(nonmask_count_buffer), 1, fp);
                                fclose(fp);
                            }
                            else if (!strcmp(names[j], "bad") || !strcmp(names[j], "none"))
                            {
                                if (nonmask_count == 1)
                                {
                                    //인식중입니다 목소리 출력
                                ...
                                PlaySound(NULL, 0, 0);
                                sprintf(mask_count_buffer, "%d", 0);
                                FILE* fp = fopen("C:\\mask_count.txt", "w");
                                fwrite(mask_count_buffer, strlen(mask_count_buffer), 1, fp);
                                fclose(fp);

                                nonmask_count++;
                                sprintf(nonmask_count_buffer, "%d", nonmask_count);
                                fp = fopen("C:\\nonmask_count.txt", "w");
                                fwrite(nonmask_count_buffer, strlen(nonmask_count_buffer), 1, fp);
                                fclose(fp);
                            }
                        ...
                        else if (mask_count == dectime)
                        {
                                    //지나가주십시오 목소리 출력
                                    PlaySound("C:\\recoend.wav", NULL, SND_FILENAME | SND_ASYNC | SND_LOOP);
                                    int reclen = 250;
                                    while (reclen > 0)
                                    {
                                        reclen--;
                                    ...
                                    ShellExecute(0, "open", "http://121.158.144.238:8000/Mask/namchuncheon1", 0, 0, SW_HIDE);
                                    printf("%s", "GoodURL!!!!!!");
                        ...
                        else if (nonmask_count==dectime)
                        {
                                //마스크 경고 출력
                                PlaySound("C:\\recoend2.wav", NULL, SND_FILENAME | SND_ASYNC | SND_LOOP);
                                int reclen = 250;
                                while (reclen > 0)
                                {
                                    reclen--;
                                ...
                                ShellExecute(0, "open", "http://121.158.144.238:8000/Mask/namchuncheon0", 0, 0, SW_HIDE);
                                printf("%s", "BADURL!!!!!!");
                                sprintf(mask_count_buffer, "%d", 0);
                                FILE* fp = fopen("C:\\mask_count.txt", "w");
                                fwrite(mask_count_buffer, strlen(mask_count_buffer), 1, fp);
